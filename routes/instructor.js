...
router.get('/my-courses', async (req, res) => {
  const rows = await db.prepare('SELECT * FROM courses WHERE instructor_id = ? ORDER BY id DESC').all(req.user.id);
...
router.post('/schedule', async (req, res) => {
  ...
  const course = await db.prepare('SELECT * FROM courses WHERE id = ? AND instructor_id = ?').get(course_id, req.user.id);
