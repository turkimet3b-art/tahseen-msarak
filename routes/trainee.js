...
router.put('/my-courses/:enrollmentId/progress', async (req, res) => {
  const { progress } = req.body || {};
  const enrollment = await db.prepare('SELECT * FROM enrollments WHERE id = ? AND trainee_id = ?').get(req.params.enrollmentId, req.user.id);
  if (!enrollment) return res.status(404).json({ error: 'التسجيل غير موجود' });
  const pct = Math.max(0, Math.min(100, Number(progress) || 0));
  const status = pct >= 100 ? 'completed' : 'active';
  await db.prepare('UPDATE enrollments SET progress = ?, status = ? WHERE id = ?').run(pct, status, req.params.enrollmentId);
  if (status === 'completed') {
    const course = await db.prepare('SELECT * FROM courses WHERE id = ?').get(enrollment.course_id);
    const already = await db.prepare('SELECT id FROM certificates WHERE trainee_id = ? AND course_id = ?').get(req.user.id, enrollment.course_id);
    if (!already) {
      await db.prepare('INSERT INTO certificates (trainee_id,course_id,title) VALUES (?,?,?)').run(
...
router.get('/notifications', async (req, res) => {
  const rows = await db.prepare('SELECT * FROM notifications WHERE user_id = ? ORDER BY id DESC LIMIT 20').all(req.user.id);
...
router.get('/next-session', async (req, res) => {
  const row = await db.prepare(
    `SELECT sessions_live.*, courses.title AS course_title FROM sessions_live
     ...
     WHERE enrollments.trainee_id = ? AND sessions_live.session_date >= CURRENT_DATE
